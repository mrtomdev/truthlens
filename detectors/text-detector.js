...
function scoreBurstiness(sentences) {
  ...
  // Human text typically has CV > 0.6. LLM output clusters around 0.25-0.45.
  // Lower variance => higher AI suspicion.
...
function scoreCommonWordRatio(tokens) {
  ...
  // LLMs lean on common words. Ratio > 0.55 is suspicious; > 0.65 is highly suspicious.
...
function scoreRepetition(tokens) {
  ...
  // LLMs repeat short bigrams less than humans paraphrasing, BUT repeat templated bigrams more.
  // This is a U-shape; score it conservatively.
...
function scoreSyntacticUniformity(sentences) {
  ...
  // High uniqueness = human. Low uniqueness = templated.
