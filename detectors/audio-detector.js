...
function estimateSpectralFlatness(samples, sr) {
  // Cheap proxy: ratio of geometric mean to arithmetic mean of |samples| windowed.
  const win = 1024;
  let totalFlatness = 0;
  let windows = 0;
  ...
  // Synthetic voices: > 0.5. Real: typically 0.2-0.4.
...
function estimateSilenceRegularity(samples, sr) {
  ...
  // Synthetic speech: highly regular pauses (CV < 0.3). Human: irregular (CV > 0.5).
...
function estimatePitchJitter(samples, sr) {
  // Zero-crossing-rate based pitch proxy. Compute per 30ms window, then std-dev.
  ...
  const cv = Math.sqrt(variance) / mean;
  // Human pitch jitters more. Low CV => synthetic.
